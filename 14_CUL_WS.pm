...
CUL_WS_Parse($$)
{
  ...
  if($sfirstbyte == 7) {
    ...
    if($typbyte == 1 && int(@a) > 8) {           # temp/hum
      ...
      $devtype = "PS50";
      $family = "WS300";
      $NotifyType="T H DP AH";
      $NotifyTemperature=$tmp;
      $NotifyHumidity=$hum;
      $NotifyDewpoint=sprintf("%.1f", calc_dewpoint($hum, $tmp));
      $NotifyAbsHumidity=sprintf("%.1f", calc_af($hum, $tmp));
      $val = sprintf("T: %.1f  H: %.1f  DP: %.1f  AH: %.1f", $tmp, $hum, $NotifyDewpoint, $NotifyAbsHumidity);
    }
  ...
  for($j=0; $j<int(@Notifies); $j++) {
    ...
    } elsif($Notifies[$j] eq "DP")  { $val = $NotifyDewpoint;
